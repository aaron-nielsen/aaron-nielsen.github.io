...
idx = which(p == "Miles Davis" | p == "Anthony Braxton" | p == "John Coltrane" |
              ...
              p == "Cecil Taylor" | p == "Albert Ayler" | p == "Duke Ellington" |
              p == "Sun Ra" )
...
top = g2[-1,]
top$PopCount[is.na(top$PopCount)]=0
...
artists$Count = as.integer(artists$Count)

ggplot(artists,aes(x=Count,y=Artist)) + 
  geom_bar(stat="identity",fill ="steelblue") + 
  scale_y_discrete(limits=rev(artists$Artist)) + 
  scale_x_continuous(breaks = seq(0,30,by=5)) + 
  theme_linedraw() + 
  theme_bw(base_size = 16) +
  geom_text(aes(label=Count),hjust=1.5, color="white", size=3.5)
```

# Jazz Albums Listened to by Release Year
```{r hist, echo = FALSE, message = FALSE, warning=FALSE}
top_idx = c(216,220,219,192,195,178,124,129,105,69)
...
ggplot(jazz_data, aes(x=Year)) +
  geom_histogram(binwidth=1,fill="#69b3a2", color="#e9ecef", alpha=0.9) +
  geom_density(aes(y=1 * ..count..),alpha=0.1, fill="gray") +
  scale_x_continuous(name="Year", breaks = seq(1900,2022,10)) +
  scale_y_continuous(name="Count") +
  theme_gray(base_size=16)
```

# Jazz Albums Listened to by Decade
```{r hist2, echo = FALSE, message = FALSE, warning=FALSE}
ggplot(jazz_data, aes(x=Year)) +
  geom_histogram(fill="#69b3a2", color="#e9ecef", alpha=0.9,breaks=seq(1939,2029,by=10)) +
  scale_x_continuous(name="Year", breaks = seq(1945,2025,10),
